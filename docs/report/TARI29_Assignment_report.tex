...
=======
\newcommand{\docauthor}{Andrej Kutny, Charles Madjeri, Samrat Debnath, \\ Vladimir Giustacchini, Aikeya Ainiwaer}
>>>>>>> 4ddef7f2c7ce329774f835549cc38ae2133ce506
...
\subsection[Experiment 1]{Tuning generation number limit over cost}
...
\begin{axis}[
  ...
  title={EIL76: Cost vs Generation Number},
]
...
\begin{axis}[
  ...
  grid=both,
  legend pos=north west,
  title={A280: Cost vs Generation Number},
]
...
\begin{axis}[
  ...
  grid=both,
  legend pos=north west,
  title={PR136: Cost vs Generation Number},
]
...
In this experiment, we tested the effect of tuning the generation number limit on the performance of an evolutionary algorithm, using three different datasets.

From the results, we can conclude the following:
There is no consistent pattern across all datasets that links higher generation numbers with lower costs.
In the PR136 dataset, increasing the generation number tends to reduce the cost, at least up to a certain point.
For EIL76 and A280, the relationship is less clear, and increasing the generation number does not always lead to better results.


To make stronger conclusions, this type of experiment should be repeated with more datasets, ideally covering a broader range of problem sizes and structures. Further studies could also test additional parameters, such as population size, to better understand how different settings interact and affect performance.
