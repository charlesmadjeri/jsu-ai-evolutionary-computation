...
class ManhattanEliteSelector(EliteSelector):
    ...
    def calculate_cost(self) -> list[float]:
        for i in range(len(self.generation)):
            ...
            for j in range(1, len(self.generation[i])):
                self.cost[i] += ManhattanCostCalculation.calculate(self, self.coordinates[self.generation[i][j-1] - 1], self.coordinates[self.generation[i][j] - 1])
        return self.cost
